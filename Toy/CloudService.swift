...
struct ToyProfile {
    ...
    var forbiddenContent =      [ForbiddenItem]()
}
...
class CloudService: NSObject, URLSessionDataDelegate{
        ...
        if JSONSerialization.isValidJSONObject(json) {
            ...
            let task = self.cloudSession!.dataTask(with: urlRequest as URLRequest){ (data, response, error) -> Void in
                DispatchQueue.main.async{
                    ...
                    guard let jsonReceived = try? JSONSerialization.jsonObject(with: data!, options: []) as? [String: AnyObject]
                        else {
                            completion(false, "makePOSTrequest -> ERROR: Received JSON is incorrect" as AnyObject?)
                            return
                    }
                    let success = jsonReceived!["success"] as? Int
                    if success != 1 {
    ...
    func processFacebookLogin(_ facebookID: String, name: String, email: String, completion: @escaping (_ success: Bool) -> Void){
        ...
        let defaults = UserDefaults(suiteName: "group.kiqtoy.com")
        var deviceToken = defaults?.object(forKey: "deviceToken") as! String?
        if deviceToken == nil {
            deviceToken = ""
        }
    ...
    func getUserData(_ completion: @escaping (_ success: Bool, _ userData: SessionSettings) -> Void){
        ...
        makePOSTrequest(json as AnyObject, url: url){ (success, response) -> Void in
            if success != true {
                #if DEBUG
                    print ("ERROR: getUserData error = ", response)
                #endif
                ...
                let forbidden = data["forbidden_tags"] as? [NSDictionary] // Parsing forbidden items
                if forbidden != nil {
                    for i in 0 ..< forbidden!.count{
                        let id  = self.getIntFromJSON(key: "id", json: forbidden![i] as! [String : AnyObject])
                        var text = self.getStringFromJSON(key: "title", json: forbidden![i] as! [String : AnyObject])
                        let bit = self.getIntFromJSON(key: "bit", json: forbidden![i] as! [String : AnyObject])
                        if id != 0 && text.isEmpty == false {
                            text = text.capitalized
                            let forbiddenItem = ForbiddenItem(id: id, text: text, on: false, bit: bit)
                            if self.localSettings?.toyProfile.forbiddenContent.isEmpty == true {
                                self.localSettings?.toyProfile.forbiddenContent.append(forbiddenItem!)
                            } else {
                                var found : Bool = false
                                for item in (self.localSettings?.toyProfile.forbiddenContent)! {
                                    if item.id == forbiddenItem!.id {
                                        found = true
                                    }
                                }
                                if found == false {
                    ...
                    if userToys != nil && (userToys?.count)! > 0 {
                        for i in 0 ..< userToys!.count {
                            ...
                            if self.localSettings?.toyProfile.toyID == toyID { // This toy was connected to the phone
                                ...
                                let forbiddenTags = userToy.value(forKey: "forbidden_tags") as? [Int]
                                if forbiddenTags != nil && self.localSettings?.toyProfile.forbiddenContent != nil {
                                    for i in 0 ..< (self.localSettings?.toyProfile.forbiddenContent.count)! {
                                        for j in 0 ..< forbiddenTags!.count {
                                            if forbiddenTags![j] ==  self.localSettings?.toyProfile.forbiddenContent[i].id {
                                                self.localSettings?.toyProfile.forbiddenContent[i].on = true
                                            }
                                        }
                                    }
                                }
    ...
    func updateUser(_ sessionSettings: SessionSettings, completion: ((_ success: Bool) -> Void)? = nil){
        ...
        makePOSTrequest(json as AnyObject, url: url){ (success, response) -> Void in
            if success != true {
                #if DEBUG
                    print ("ERROR: updateUser error = ", response)
                #endif
            }
            completion?(success)
        }
    ...
    func addToy(_ sessionSettings: SessionSettings, completion: @escaping (_ success: Bool) -> Void){
        ...
        makePOSTrequest(json as AnyObject, url: url){ (success, response) -> Void in
            if success != true {
                #if DEBUG
                    print ("ERROR: addToy error = ", response)
                #endif
            } else {
    ...
    func updateToy(_ sessionSettings: SessionSettings){
        ...
        var forbidden = [String : AnyObject]()
        var forbiddens = [[String : AnyObject]()]

        for i in 0 ..< localSettings!.toyProfile.forbiddenContent.count {
            let item = localSettings!.toyProfile.forbiddenContent[i]
            forbidden.updateValue(item.id as AnyObject, forKey: "id")
            forbidden.updateValue(item.on as AnyObject, forKey: "value")
            forbidden.updateValue(item.bit as AnyObject, forKey: "bit")
            if i == 0 {
                forbiddens[0] = forbidden
            } else {
        ...
        let json: [String : AnyObject] = [
            ...
            //"nrf":          Int(sessionSettings.toyProfile.deviceInfo.NRFversion) as AnyObject,
            //"version":         Int(sessionSettings.toyProfile.deviceInfo.packVersion)as AnyObject,
            "user_preferences": forbiddens as AnyObject
        ]
        success = true
        #if DEBUG
        ...
        makePOSTrequest(json as AnyObject, url: url){ (success, response) -> Void in
            if success != true {
                #if DEBUG
                    print ("ERROR: updateToy error = ", response)
                #endif
    ...
    func updateJokes(jokes: [RecentItem], completion: @escaping (_ success: Bool, _ response: [RecentItem]?) -> Void){
        ...
        makePOSTrequest(json as AnyObject, url: url){ (callbackSuccess, response) -> Void in
            ...
            if success != true {
                #if DEBUG
                    print ("ERROR: updateJokes error = ", response)
                #endif
            } else {
                success = false
    ...
    func logMessageToCloud (_ message : String){
        ...
        makePOSTrequest(json as AnyObject, url: url){ (success, response) -> Void in
            if success != true {
                #if DEBUG
                    print ("ERROR: logMessageToCloud error = ", response)
                #endif
            }
        }
    ...
    func setLoggedIn (_ clientID : UInt32, loginType: String){
        ...
        let defaults = UserDefaults(suiteName: "group.kiqtoy.com")
        defaults?.set(Int(clientID), forKey: "clientID")
        defaults?.set(loginType, forKey: "loginType");
    ...
    func getBaseInfo () {
        ...
        guard let defaults = UserDefaults(suiteName: "group.kiqtoy.com")
            else {return}
        
        let clientID = defaults.integer(forKey: "clientID")
        localSettings!.clientID = UInt32(clientID)
        
        localSettings!.offlineCahngeMade = defaults.bool(forKey: "offlineChange")
        
        guard let toyID = defaults.string(forKey: "toyID"),
            let toyName = defaults.string(forKey: "toyName"),
            let silentStart = defaults.string(forKey: "silentStart"),
            let silentEnd = defaults.string(forKey: "silentEnd"),
            let sessionType = defaults.string(forKey: "loginType")
            else { return }
        
        let ESPversion = defaults.integer(forKey: "ESPversion")
        let model = defaults.integer(forKey: "model")
        let NRFversion = defaults.integer(forKey: "NRFversion")
        let packVersion = defaults.integer(forKey: "packVersion")
        let revision = defaults.integer(forKey: "revision")
        
        localSettings!.toyProfile.notifSettings.removeAll()
        let phoneCount = defaults.integer(forKey: "phoneCount")
        for i in 0 ..< phoneCount {
            let notification = NotificationItem(text: "", on: true, bit: 0)!
            if self.localSettings?.toyProfile.notifSettings.count == 0 {
                self.localSettings?.toyProfile.notifSettings.append([NotificationItem]())
            }
            notification.on =  defaults.bool(forKey: "phoneOn" + String(i))
            notification.bit =  defaults.integer(forKey: "phoneBit" + String(i))
            let text = defaults.string(forKey: "phoneText" + String(i))
            if text != nil {
        ...
        let messageCount = defaults.integer(forKey: "messageCount")
        for i in 0 ..< messageCount {
            let notification = NotificationItem(text: "", on: true, bit: 0)!
            if self.localSettings?.toyProfile.notifSettings.count == 1 {
                self.localSettings?.toyProfile.notifSettings.append([NotificationItem]())
            }
            notification.on =  defaults.bool(forKey: "messageOn" + String(i))
            notification.bit =  defaults.integer(forKey: "messageBit" + String(i))
            let text = defaults.string(forKey: "messageText" + String(i))
            if text != nil {
                notification.text = text!
            }
            self.localSettings?.toyProfile.notifSettings[1].append(notification)
        ...
        let otherCount = defaults.integer(forKey: "otherCount")
        for i in 0 ..< otherCount {
            let notification = NotificationItem(text: "", on: true, bit: 0)!
            if self.localSettings?.toyProfile.notifSettings.count == 2 {
                self.localSettings?.toyProfile.notifSettings.append([NotificationItem]())
            }
            notification.on =  defaults.bool(forKey: "otherOn" + String(i))
            notification.bit =  defaults.integer(forKey: "otherBit" + String(i))
            let text = defaults.string(forKey: "otherText" + String(i))
            if text != nil {
                notification.text = text!
            }
            self.localSettings?.toyProfile.notifSettings[2].append(notification)
        ...
        localSettings!.toyProfile.forbiddenContent.removeAll()
        let forbiddenCount = defaults.integer(forKey: "forbiddenCount")
        for i in 0 ..< forbiddenCount {
            let forbidden = ForbiddenItem(id: 0, text: "", on: true, bit: 0)!
            forbidden.on =  defaults.bool(forKey: "forbiddenOn" + String(i))
            forbidden.bit =  defaults.integer(forKey: "forbiddenBit" + String(i))
            forbidden.id =  defaults.integer(forKey: "forbiddenId" + String(i))
            let text = defaults.string(forKey: "forbiddenText" + String(i))
            if text != nil {
                forbidden.text = text!
            }
            self.localSettings?.toyProfile.forbiddenContent.append(forbidden)
        ...
        localSettings!.volume = UInt8(defaults.integer(forKey: "volume"))
        localSettings!.voiceType = UInt8(defaults.integer(forKey: "voiceType"))
        ...
        localSettings!.silentSettings.isDndOn = defaults.bool(forKey: "silentEnabled")
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "HH:mm"
        let locale = Locale.init(identifier: "en_US_POSIX")
        dateFormatter.locale = locale
    ...
    func saveBaseInfo (){
        let defaults = UserDefaults(suiteName: "group.kiqtoy.com")
        defaults?.set(String(self.localSettings!.toyProfile.toyID), forKey: "toyID");
        defaults?.set(String(self.localSettings!.toyProfile.toyName), forKey: "toyName");
        defaults?.set(Int(self.localSettings!.volume), forKey: "volume");
        defaults?.set(Int(self.localSettings!.voiceType), forKey: "voiceType");
        defaults?.set(self.localSettings!.offlineCahngeMade, forKey: "offlineChange")
        
        defaults?.set(Int(self.localSettings!.toyProfile.deviceInfo.ESPversion), forKey: "ESPversion");
        defaults?.set(Int(self.localSettings!.toyProfile.deviceInfo.model), forKey: "model");
        defaults?.set(Int(self.localSettings!.toyProfile.deviceInfo.NRFversion), forKey: "NRFversion");
        defaults?.set(Int(self.localSettings!.toyProfile.deviceInfo.packVersion), forKey: "packVersion");
        defaults?.set(Int(self.localSettings!.toyProfile.deviceInfo.revision), forKey: "revision");
        
        if localSettings!.toyProfile.notifSettings.count == 3 {
            defaults?.set(localSettings!.toyProfile.notifSettings[0].count, forKey: "phoneCount");
            for i in 0 ..< localSettings!.toyProfile.notifSettings[0].count {
                let notification = localSettings!.toyProfile.notifSettings[0][i]
                defaults?.set(String(notification.text), forKey: "phoneText" + String(i));
                defaults?.set(notification.on, forKey: "phoneOn" + String(i));
                defaults?.set(notification.bit, forKey: "phoneBit" + String(i));
            }
            defaults?.set(localSettings!.toyProfile.notifSettings[1].count, forKey: "messageCount");
            for i in 0 ..< localSettings!.toyProfile.notifSettings[1].count {
                let notification = localSettings!.toyProfile.notifSettings[1][i]
                defaults?.set(String(notification.text), forKey: "messageText" + String(i));
                defaults?.set(notification.on, forKey: "messageOn" + String(i));
                defaults?.set(notification.bit, forKey: "messageBit" + String(i));
            }
            defaults?.set(localSettings!.toyProfile.notifSettings[2].count, forKey: "otherCount");
            for i in 0 ..< localSettings!.toyProfile.notifSettings[2].count {
                let notification = localSettings!.toyProfile.notifSettings[2][i]
                defaults?.set(String(notification.text), forKey: "otherText" + String(i));
                defaults?.set(notification.on, forKey: "otherOn" + String(i));
                defaults?.set(notification.bit, forKey: "otherBit" + String(i));
            }
        }
        
        defaults?.set(localSettings!.toyProfile.forbiddenContent.count, forKey: "forbiddenCount");
        for i in 0 ..< localSettings!.toyProfile.forbiddenContent.count {
            let forbidden = localSettings!.toyProfile.forbiddenContent[i]
            defaults?.set(String(forbidden.text), forKey: "forbiddenText" + String(i));
            defaults?.set(forbidden.on, forKey: "firbiddenOn" + String(i));
            defaults?.set(forbidden.id, forKey: "forbiddenId" + String(i));
            defaults?.set(forbidden.bit, forKey: "forbiddenBit" + String(i));
        }
        ...
        let locale = Locale.init(identifier: "en_US_POSIX")
        dateFormatter.locale = locale
        defaults?.set(dateFormatter.string(from: (self.localSettings!.silentSettings.start)!),forKey: "silentStart")
        defaults?.set(dateFormatter.string(from: (self.localSettings!.silentSettings.end)!),forKey: "silentEnd")
        defaults?.set(self.localSettings!.silentSettings.isDndOn, forKey: "silentEnabled")
    ...
    func offlineChangeMade (_ status: Bool){
        let defaults = UserDefaults(suiteName: "group.kiqtoy.com")
        defaults?.set(status, forKey: "offlineChange")
    }
}
