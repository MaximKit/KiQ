...
class CentralViewController: UIViewController, CBCentralManagerDelegate, CBPeripheralDelegate  {
    ...
    var toyTabBarController: MainTabBarController?
    ...
    private var bleStatus: BLEStatus = .off {
        didSet {
            ...
            case .on:
                DispatchQueue.main.async{
                    ...
                    if self.toyStatus == .disconnected && self.sessionSettings.clientID != 0  {
                        if self.expectedToyID.isEmpty == false || self.cloundService.isCloudSessionEnabled() == true {
                            #if DEBUG
                                print("DBG: BLE = .On: -> .Idle -> .Searching")
                            #endif
                            self.toyStatus = ToyStatus.searching
                        }
                        
                        if self.expectedToyID.isEmpty == true && self.cloundService.isCloudSessionEnabled() == false {
                            #if DEBUG
                                print("DBG: BLE = .On, .Idle , no cloud, toyID is empthy -> stay .Idle")
                            #endif
            ...
            case .off:
                DispatchQueue.main.async{
                    #if DEBUG
                        print("DBG: BLE = .Off: -> Go .Idle")
                    #endif
                    if self.cloundService.isCloudSessionEnabled() == false {
                        self.sessionSettings = self.cloundService.getCloudBaseInfo()
                    }
                    if self.sessionSettings.toyProfile.toyID.isEmpty == false {
                        self.toyStatus = ToyStatus.disconnected
                        self.upperStatusLabel = "The toy is disconnected, turn on Bluetooth to connect"
                        self.myToyViewController?.bleDidChange(false)
                    } else {
                        self.upperStatusLabel = "Turn on Bluetooth to connect a new toy"
                        self.myToyViewController?.bleDidChange(false)
                    }
        ...
        didSet {
            ...
            case ToyStatus.searching:
                DispatchQueue.main.async{
                    ...
                    #endif
                    self.toyTabBarController?.diasbleResentsTabBarItem()
                    self.toyTabBarController?.selectedIndex = 0
                    self.toyTabBarController?.navigationItem.rightBarButtonItem?.isEnabled = false
                    if self.expectedToyID == "" {
                        self.myToyViewController?.ConnectAnotherButton.isHidden = true
                        self.upperStatusLabel = "Connecting to a new toy..."
                    } else {
                    ...
                    self.myToyViewController?.bleDidChange(true)
                    self.scanForPeripheral()

                    if self.toyRescanTimer == nil {
                        self.toyRescanTimer = Timer.scheduledTimer(timeInterval: 0.5, target: self, selector: #selector(CentralViewController.scanForPeripheral), userInfo: nil, repeats: true)
                    } else {
                        #if DEBUG
                            print("ERROR: Double Go -> ToyStatus.searching - this is wrong, take a look")
                        #endif
                    }
                }
                break
            ...
            case ToyStatus.connected:  //CONNECTED ---------------------------------
                DispatchQueue.main.async{
                    ...
                    self.toyTabBarController?.navigationItem.rightBarButtonItem?.isEnabled = true
            ...
            case ToyStatus.disconnected:  // IDLE ---------------------------
                DispatchQueue.main.async{
                    ...
                    recents.removeAll()
                    self.recentFilesViewController?.RecentItemsTable.reloadData()
    ...
    var internetConnectionStatus: InternetConnectionStatus = .notConnected {
        didSet {
            if internetConnectionStatus != previousInternetConnectionStatus{
                ...
                #if DEBUG
                    print ("DBG: internetConnectionStatus:", internetConnectionStatus)
                #endif
                ...
                case .notConnected:
                    DispatchQueue.main.async{
                        ...
                        self.toyTabBarController?.navigationItem.leftBarButtonItem?.isEnabled = false
    ...
    func startBLEConnection (_ settings: SessionSettings){
        #if DEBUG
            logMessageToCloud("DBG: startBLEConnection")
        #endif
        ...
        self.myToyViewController?.cloudDidChange(false)
        self.userSettingsViewController?.tableView.reloadData()
        self.toyTabBarController?.navigationItem.leftBarButtonItem?.isEnabled = true
        if self.bleStatus == BLEStatus.on {
            ...
            case ToyStatus.disconnected:
                if self.bleStatus == BLEStatus.on {
                    #if DEBUG
                        print("DBG: startBLEConnection: .disconnected -> .searching")
                    #endif
                    self.expectedToyID = self.sessionSettings.toyProfile.toyID
                    self.toyStatus = ToyStatus.searching
                }
            ...
            case ToyStatus.searching:
                #if DEBUG
                    print("DBG: startBLEConnection: already .searching")
                #endif
                if self.expectedToyID == "" {
                    self.myToyViewController?.ConnectAnotherButton.isHidden = true
                    self.upperStatusLabel = "Connecting to a new Toy..."
            ...
            case ToyStatus.connected:
                #if DEBUG
                    print("DBG: startBLEConnection: already .connected")
                #endif
                        ...
                        self.cloundService.updateJokes(jokes: recents){ (success, response) -> Void in
                             DispatchQueue.main.async{
                                ...
                                if success != true {
                                    ...
                                    recents = response!
                                    self.lowerStatusLabel = "\n"
                                    self.myToyViewController?.lastPlayedDidChange(recents[0])
                                    if recents[0].rate != 0 {
                                        if recents[0].rate == 1 {
                                            self.myToyViewController?.LikedCatPicture.isHidden = false
                                            self.myToyViewController?.LikeButton.setTitle("LIKED", for: UIControlState())
                                            self.myToyViewController?.DislikedCatPicture.isHidden = true
                                            self.myToyViewController?.DislikeButton.setTitle("DISLIKE", for: UIControlState())
                                            
                                        } else if recents[0].rate == -1 {
                                            self.myToyViewController?.LikedCatPicture.isHidden = true
                                            self.myToyViewController?.LikeButton.setTitle("LIKE", for: UIControlState())
                                            self.myToyViewController?.DislikedCatPicture.isHidden = false
                                            self.myToyViewController?.DislikeButton.setTitle("DISLIKED", for: UIControlState())
                                        }
                                    } else {
                                        self.myToyViewController?.LikedCatPicture.isHidden = true
                                        self.myToyViewController?.DislikedCatPicture.isHidden = true
                                    ...
                                    self.recentFilesViewController?.RecentItemsTable.reloadData()
                                    self.toyTabBarController?.enableResentsTabBarItem()
                                }
                            }
                        }
                    }
        ...
        case ToyStatus.connected:
            ...
            for service in peripheral.services! {
                ...
                case CONTENT_SERVICE_UUID:
                    peripheral.discoverCharacteristics([CONT_PLAYFYLE_C, CONT_FILESPLAYED_C, CONT_SENDFILE_C], for: thisService)
                    break
                ...
                for charactericsx in charactericsArr
                {
                    ...
                    case SETUP_SETTINGS_C:
                        ...
                        if expectedToyID == "" {
                            ...
                            var forbiddenBLE: Int = 0
                            for forbidden in sessionSettings.toyProfile.forbiddenContent {
                                if forbidden.on == true {
                                    forbiddenBLE |= (1 << forbidden.bit)
                                }
                            }
                            ...
                            silentPeriodBLE.forbidden = UInt32(forbiddenBLE)
        ...
        if toyStatus == ToyStatus.searching {
            ...
            if characteristic.uuid == HS_DEVICEINFO_C { // The some Toy has sent to us its ToyID
                ...
                if  discoveredPerifCharxs[peripheral]?.toyID == expectedToyID || expectedToyID == ""{
                    ...
                    discoveredPerifCharxs[peripheral]?.deviceInfo?.ESPversion = buffer16[4]
                    discoveredPerifCharxs[peripheral]?.deviceInfo?.NRFversion = buffer16[5]
                    discoveredPerifCharxs[peripheral]?.deviceInfo?.packVersion = buffer16[6]
        ...
        // *********** Check if we are connected to the Toy ******************
            ...
            case WIFI_UPGRADE_C:
                ...
                if toyUpgradeStatus != ToyUpgradeStatus.upgradeIdle {
                    ...
                    #if DEBUG
                        logMessageToCloud("DBG: .connected, upgrade received: -> .upgradeIdle")
                    #endif
                }
                break
            ...
            case SETUP_STATUS_C:
                ...
                if array.count >= 3 {
                    ...
                    myToyViewController?.toyStatusDidChange(batteryLevel: toyBatteryLevel, isCharging: (toyCharx?.isCharging)!, isSilent: (toyCharx?.isSilent)!)
                }
                break
            ...
            case CONT_FILESPLAYED_C:
                ...
                if numOfFiles == 0 { // numOfFiles is 0 if this is notification about just played file
                    #if DEBUG
                        logMessageToCloud ("DBG: Last file played received")
                    #endif

                    let recentItem = RecentItem(text: "", fileID: buffer32[1], maleFileID: 0, femaleFileID: 0, rate: 0, url: "https://kiqtoy.com")
                    if recentItem?.fileID != 0 {
                        if cloundService.isCloudSessionEnabled() == true {  // Cloud is available
                            cloundService.updateJokes(jokes: [recentItem!]) { (success, response) -> Void in  // Updating last file played by text
                                DispatchQueue.main.async{
                                    if success != true {
                                        self.logMessageToCloud ("ERROR: get text for last file played failed, fileID = " +
                                            String(describing: recentItem?.fileID))
                                        ...
                                        if response?.isEmpty == false && response?[0].text.isEmpty == false{
                                            ...
                                            recents.insert(response![0], at: 0)
                    ...
                    } else {  // File ID must not be 0.
                        logMessageToCloud("ERROR: last played file ID is empty")
                        if self.last10IsFetching == false && self.last10ReceivedButNotUpdated == false {
                            getLast10Played()
                        }
                    }
                } else { // last 10 played files received
                    if fileOffset == 0 { // First portion of the last 10 received
                        recents.removeAll()
                    }
                    ...
                    for i in 0...3 {
                        ...
                        recent = RecentItem(text: "", fileID: buffer32[i + 1], maleFileID: 0, femaleFileID: 0, rate: 0, url: "https://kiqtoy.com")
                        if recent?.fileID != 0 {
                            recents.insert(recent!, at: 0)
                        }
                    
                        if Int(fileOffset) + i == Int(numOfFiles) - 1 { //All last file played received
                            if recents.isEmpty != true && recents[0].fileID != 0 {
                                ...
                                #if DEBUG
                                    logMessageToCloud("DBG: Received list of last " + String(numOfFiles) + " files played")
                                #endif
                                                ...
                                                if recents.isEmpty == false && recents[0].text.isEmpty == false {
                                                    ...
                                                    if recents[0].rate != 0 {
                                                        if recents[0].rate == 1 {
                                                            self.myToyViewController?.LikeButton.setTitle("LIKED", for: UIControlState())
                                                            self.myToyViewController?.LikedCatPicture.isHidden = false
                                                            self.myToyViewController?.LikeButton.isUserInteractionEnabled = true
                                                        } else if recents[0].rate == -1 {
                                                            self.myToyViewController?.DislikedCatPicture.isHidden = false
                                                            self.myToyViewController?.DislikeButton.setTitle("DISLIKED", for: UIControlState())
                                                            self.myToyViewController?.DislikeButton.isUserInteractionEnabled = true
                                                        }
                                                    } else {
                                                        self.myToyViewController?.LikedCatPicture.isHidden = true
                                                        self.myToyViewController?.DislikedCatPicture.isHidden = true
                                                    ...
                                                    self.recentFilesViewController?.RecentItemsTable.reloadData()
                                                    self.toyTabBarController?.diasbleResentsTabBarItem()
                                                    self.logMessageToCloud("ERROR: get text for last 10 files played -> response is empthy")
                                                }
                                            }
                                            self.upperStatusLabel = self.sessionSettings.toyProfile.toyName + " is connected"
                                        }
    ...
    func theToyIsFound (_ peripheral: CBPeripheral) {
        #if DEBUG
            logMessageToCloud("DBG: " + String(sessionSettings.toyProfile.toyID) + "is being connected")
        #endif
        self.myBLECentralManager.stopScan()
        ...
        if expectedToyID == ""{
            ...
            self.logMessageToCloud("NORMAL: Known Toy " + String(self.sessionSettings.toyProfile.toyID) + " is connected")
            self.toyStatus = ToyStatus.connected
            // Request services for the Toy
            peripheral.discoverServices([SETUP_SERVICE_UUID, CONTENT_SERVICE_UUID, WIFI_SERVICE_UUID])
            ...
            if(connectedPeripherals.count != 0){
                for peripheral in connectedPeripherals {
                    if !discoveredPeripherals.contains(peripheral) {
                        ...
                        self.myBLECentralManager.connect(peripheral, options: nil)
                        #if DEBUG
                            logMessageToCloud("DBG: scanForPeripheral: " + String(describing: peripheral.name) + " already paired. Connecting...")
                        #endif
                    }
                }
            }
    ...
    func toyNameDidChange (_ name: String)
    {
        ...
        if self.cloundService.isCloudSessionEnabled() == false {
            self.upperStatusLabel = self.sessionSettings.toyProfile.toyName + " is connected"
            self.myToyViewController?.bleDidChange((self.myToyViewController?.FetchingDataIndicator.isAnimating)!)
        }
    }
    ...
    func generalSettingDidChange(){
        ...
        if toyStatus == .connected && toySuplCharx?.setup.settingsCharx != nil {
            ...
            settingsBLE.forbidden = UInt32(forbiddenBLE)
    ...
    func getLast10Played(){
        ...
        DispatchQueue.main.async{
            ...
            if self.toyStatus == .connected && self.toySuplCharx?.content.sendFileCharx != nil{
                let dataToWrite = self.encode(UInt8 (1))
                ...
                self.getLast10TimeoutTimer = Timer.scheduledTimer(timeInterval: 25, target: self, selector: #selector(CentralViewController.last10FetchingTimeout), userInfo: nil, repeats: false)
                #if DEBUG
                    self.logMessageToCloud("DBG: getLast10Played: last 10 requested, timer created")
                #endif
            }
        }
    }
    ...
    //-----------------------------------------------------------------------
    func getWiFiStatusTimeoutOccured(){
        #if DEBUG
    ...
    //-----------------------------------------------------------------------
    func getKnownWiFiTimeoutOccured(){
        #if DEBUG
    ...
    //-----------------------------------------------------------------------
    func getVisibleWiFiTimeoutOccured(){
        #if DEBUG
