...
class UpgradeToyTableViewController: UITableViewController{
    ...
    func updateToyFirware(){
        if centralController.isCharging() == true {
            centralController.updateToyFirmware()
        } else {
            displayMyAlertMessage("The Toy must be connected to the charger during the upgrade process. Please connect and try again.")
        }
    }
